...
class _ReferralScreenState extends State<ReferralScreen> {
  ...
  bool _loadingReferredBy = false;

  Future<void> _loadReferredBy() async {
    setState(() {
      _loadingReferredBy = true;
    });
    ...
    if (user != null) {
      ...
      if (referredBy != null) {
        final refUserDoc = await FirebaseFirestore.instance
            .collection('users')
            .doc(referredBy)
            .get();
        setState(() {
          _referredByName = refUserDoc.data()?['name'] ?? referredBy;
        });
      }
    }
    setState(() {
      _loadingReferredBy = false;
    });
  }
